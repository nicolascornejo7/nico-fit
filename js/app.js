...
async function saveReadiness(){const date=currentContext().dateKey,rec={date,sleep:+$('sleep').value,energy:+$('energy').value,fatigue:6-(+$('freshness').value),pain:+$('pain').value,painArea:$('painArea').value,updatedAt:nowIso()},validation=validateReadiness(rec);if(!validation.valid)return alert(validation.message);data.readiness=data.readiness.filter(x=>x.date!==date);data.readiness.push(rec);persistAndRender();try{await sync.syncRecord();}catch{}}
async function saveMatch(){const date=currentContext().dateKey,rec={date,energy:+$('matchEnergy').value,legs:+$('legs').value,performance:+$('performance').value,notes:$('matchNotes').value,updatedAt:nowIso()},validation=validateMatch(rec);if(!validation.valid)return alert(validation.message);data.matches=data.matches.filter(x=>x.date!==date);data.matches.push(rec);persistAndRender();try{await sync.syncRecord();}catch{}alert('Partido guardado');}
