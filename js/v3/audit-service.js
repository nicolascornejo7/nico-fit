...
import {rolloutAllowsRemote} from './rollout-state.js';
...
export class V3AuditService{
  ...
  async flush(){
    if(!isV3AuditEnabled(this.flagStorage))return {skipped:'disabled'};if(!this.remote)return {skipped:'unconfigured'};if(!await rolloutAllowsRemote())return {skipped:'rollout_blocked'};
    return withV3SyncLock({repository:this.repository,userId:this.repository.userId,locks:this.locks,task:async()=>{
      ...
      for(const event of await this.events()){
        if(!await rolloutAllowsRemote())return {sent,failed,skipped:'rollout_blocked'};
