...
import {rolloutSnapshot} from './rollout-state.js';
...
export class V3ObservabilityService{
  ...
  async snapshot(){
    ...
    const rollout=rolloutSnapshot(),online=!!this.online(),syncEnabled=rollout?rollout.remoteWritesAllowed:flagState[flags.V3_SYNC_FLAG],configured=!!this.engine,openConflicts=conflicts.filter(row=>row.status==='open').length;
    ...
    const summary=rollout?.reason||(!online?'Estás offline. Tus cambios locales se conservan.':!syncEnabled?'Sync V3 está desactivado. Los datos siguen guardados localmente.':!configured?'Falta configurar explícitamente el motor V3 de staging.':operational.lock.active?'Otra instancia puede estar sincronizando.':openConflicts?`${openConflicts} conflictos necesitan una decisión explícita.`:lastAttempt?.status==='failed'?'El último intento no terminó correctamente. Los cambios locales se conservan.':operational.queue.operations?`${operational.queue.operations} operaciones siguen pendientes.`:lastSuccessful?'No hay operaciones pendientes. Hay confirmación de un ciclo remoto exitoso.':'Todavía no hay un sync remoto confirmado.');
    return {online,summary,lastAttempt,lastSuccessfulSyncAt:lastSuccessful?.finishedAt||null,lastError,lastErrorAt:errorAttempt?.finishedAt||errorAttempt?.startedAt||null,checkpoints:Object.fromEntries(Object.entries(operational.checkpoints).map(([entity,cursor])=>[entity,{updatedAt:stamp(cursor.updatedAt),id:id(cursor.id)}])),operations:operational.counts,entities:operational.entities,queue:operational.queue,backoff:operational.backoff,lock:operational.lock,openConflicts,resolutionPending:conflicts.filter(row=>row.status==='resolution_pending').length,recentResolutions:resolutions,flags:flagState,rollout,pwa:await pwaVersionSnapshot(),actions:{syncNow:online&&syncEnabled&&configured&&!operational.lock.active,retryFailed:online&&syncEnabled&&configured&&!operational.lock.active&&operational.backoff.eligibleFailed>0},audit:await this.audit?.snapshot()||{enabled:flagState[flags.V3_AUDIT_FLAG],configured:false}};
  }
  async syncNow(){
