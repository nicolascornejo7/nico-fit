...
export function v3Progression({exercise,history=[],session,readinessScore=null,now=new Date(),occurrenceIndex=0}){
  ...
  const previous=history.filter(item=>(!session.routine_id||item.session.routine_id===session.routine_id&&item.session.routine_version===session.routine_version)&&item.session.id!==session.id&&item.session.status==='completed'&&!item.session.deleted_at&&item.session.started_at<session.started_at)
