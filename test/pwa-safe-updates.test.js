...
test('worker precaches full version, avoids automatic takeover and cleans only Nico Fit caches',async()=>{
  ...
  assert.match(sw,/const BUILD_ID='nico-fit-v22'/);
  assert.match(index,/nico-fit-build" content="nico-fit-v22"/);
  assert.match(inventory,/nico-fit-build" content="nico-fit-v22"/);
...
test('waiting worker cache survives cleanup by the active worker',async()=>{
  ...
  try{
    ...
    assert.ok(buckets.get('nico-fit-v22').has('http://127.0.0.1:8199/index.html'));
    await cacheStorage.open('nico-fit-v23');await cacheStorage.open('unrelated-cache');await cacheStorage.open('nico-fit-v21');
    ...
    assert.deepEqual((await cacheStorage.keys()).sort(),['nico-fit-v22','nico-fit-v23','unrelated-cache']);
