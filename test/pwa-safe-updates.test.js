...
test('worker precaches full version, avoids automatic takeover and cleans only Nico Fit caches',async()=>{
  ...
  assert.match(sw,/const BUILD_ID='nico-fit-v47'/);
  assert.match(index,/nico-fit-build" content="nico-fit-v47"/);
  assert.match(index,/PRUEBA v47/);
  assert.match(inventory,/nico-fit-build" content="nico-fit-v47"/);
...
test('waiting worker cache survives cleanup by the active worker',async()=>{
  ...
  try{
    ...
    assert.ok(buckets.get('nico-fit-v47').has('http://127.0.0.1:8199/index.html'));
    ...
    await cacheStorage.open('nico-fit-v48');await cacheStorage.open('unrelated-cache');await cacheStorage.open('nico-fit-v24');
    ...
    assert.deepEqual((await cacheStorage.keys()).sort(),['nico-fit-v47','nico-fit-v48','unrelated-cache']);
