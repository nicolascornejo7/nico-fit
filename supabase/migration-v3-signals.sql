...
begin
  ...
  if tg_table_name='match_reviews' then
   ...
   if linked_football_session_id is not null and nullif(row_data->>'deleted_at','') is null then
    if not exists(select 1 from nico_fit_v3.football_sessions f where f.id=linked_football_session_id and f.user_id=row_user_id and f.local_date=row_local_date and f.session_type='match' and f.deleted_at is null) then
  ...
  if tg_table_name='football_sessions' then
    if row_data->>'session_type'<>'match' and nullif(row_data->>'deleted_at','') is null and exists(select 1 from nico_fit_v3.match_reviews r where r.football_session_id=row_id and r.user_id=row_user_id and r.deleted_at is null) then
